...
def _verify_id(id: str, name: str, control_dict: Dict[str, UIAWrapper], ui_state=None):

    if not id:
        raise ToolError("Window id is required for select_application_window")

    if not control_dict:
        if ui_state is not None and ui_state.selected_app_window is not None:
            ui_state.logger.info("control_dict is empty, auto-refreshing from selected_app_window...")
            try:
                controls_list = ui_state.control_inspector.get_control_list(
                    ui_state.selected_app_window,
                    control_type_list=configs.get("CONTROL_LIST", []),
                    class_name_list=configs.get("CONTROL_LIST", []),
                )
                ui_state.control_dict = {str(i + 1): control for i, control in enumerate(controls_list)}
                control_dict = ui_state.control_dict
                ui_state.logger.info(f"Auto-refreshed control_dict with {len(control_dict)} controls")
            except Exception as e:
                ui_state.logger.warning(f"Auto-refresh failed: {e}")
        if not control_dict:
            raise ToolError(
...
def create_app_action_mcp_server(*args, **kwargs) -> FastMCP:
    ...
    def click_input(
        ...
        control_verified = _verify_id(id, name, ui_state.control_dict, ui_state)
