...
class AppScreenshotCaptureStrategy(BaseProcessingStrategy):
    ...
    async def execute(
        ...
        """
        # Check if verification is needed - skip screenshot if not
        needs_verification = context.global_context.get(ContextNames.NEEDS_VERIFICATION) if hasattr(context, 'global_context') else True
        if not needs_verification:
            self.logger.info("Skipping screenshot capture [no-verify mode]")
...
class AppControlInfoStrategy(BaseProcessingStrategy):
    ...
    async def execute(
        ...
        """
        # Check if verification is needed - skip control info collection if not
        needs_verification = context.global_context.get(ContextNames.NEEDS_VERIFICATION) if hasattr(context, 'global_context') else True
        if not needs_verification:
            self.logger.info("Skipping control info collection [no-verify mode]")
            return ProcessingResult(success=True, data={
                "control_list": [],
...
class AppLLMInteractionStrategy(BaseProcessingStrategy):
    ...
    async def execute(
        ...
        """
        # Check if verification is needed - skip screenshots if not
        needs_verification = context.global_context.get(ContextNames.NEEDS_VERIFICATION) if hasattr(context, 'global_context') else True
        skip_visual = not needs_verification
        if skip_visual:
            self.logger.info("Running LLM in text-only mode [no-verify]")
        ...
        try:
            ...
            # Step 1: Collect image strings (skip for [no-verify] mode)

            if skip_visual:
                self.logger.info("Skipping screenshot collection [no-verify mode]")
                image_string_list = []
                concat_screenshot_path = ""
                last_control_screenshot_path = ""
                knowledge_retrieved = self._knowledge_retrieval(agent, subtask) if subtask else None
            else:
