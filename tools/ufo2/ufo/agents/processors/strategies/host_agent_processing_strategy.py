...
class HostLLMInteractionStrategy(BaseProcessingStrategy):
    ...
    def _extract_structured_response_data(
        ...
        # Auto-detect needs_verification: check LLM output first, then fallback to pattern matching
        needs_verification = getattr(response, 'needs_verification', None)
        if needs_verification is None and response.current_subtask:
            subtask_lower = response.current_subtask.lower()
            no_verify_patterns = [
                "ctrl+v", "paste", "click on the chat", "click on the search box",
                "keyboard shortcut", "press enter", "click the contact",
                "click chat", "click search"
            ]
            needs_verification = not any(p in subtask_lower for p in no_verify_patterns)
        elif needs_verification is None:
