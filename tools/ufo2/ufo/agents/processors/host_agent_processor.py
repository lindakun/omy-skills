...
class HostAgentProcessor(ProcessorTemplate):
    ...
    def _finalize_processing_context(
        ...
        try:
            ...
            # Update NEEDS_VERIFICATION if available
            needs_verification = processing_context.get_local("needs_verification")
            if needs_verification is not None:
                self.global_context.set(ContextNames.NEEDS_VERIFICATION, needs_verification)
