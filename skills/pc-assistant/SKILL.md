...
**注意**：凡是涉及「手机」/「手机上」的操作，应使用 `mobile-assistant`，而非本技能。
...
1. `UFO_ROOT` 目录存在
2. `UFO_PYTHON` 存在
3. `{UFO_ROOT}/config/ufo/agents.yaml` 存在，且 API Key **不是**占位符 `YOUR_VOLC_ARK_API_KEY`
4. `agents.yaml` 含四个 agent：`HOST_AGENT`、`APP_AGENT`、`EVALUATION_AGENT`、`BACKUP_AGENT`
5. 当前系统为 Windows（UFO² 依赖 UI Automation）
